...
test('local HTTP UI serves real state, separates messages/assignments and rejects forged or cross-origin writes', async t => {
  ...
  const html = await fetch(url); assert.equal(html.status, 200); assert.match(await html.text(), /Bot Messenger/);
  ...
  const post = (path: string, value: unknown, headers: Record<string, string> = {}) => fetch(`${url}/api/${path}`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-Bot-Messenger-Token': csrfToken, ...headers }, body: JSON.stringify(value) });
  ...
  assert.equal((await post('messages', { body: 'No token' }, { 'X-Bot-Messenger-Token': 'wrong' })).status, 403);
